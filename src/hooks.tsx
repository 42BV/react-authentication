...
export function useAuthentication<User>(): AuthenticationState<User> {
  ...
  const [state, setState] = useState<AuthenticationState<User>>(service.getState());
...
export function useCurrentUser<User>(): User {
  ...
  if (!state.isLoggedIn) {
    throw new Error('@42bv/authentication: Asked for current user whilst not logged in');
  } else {
